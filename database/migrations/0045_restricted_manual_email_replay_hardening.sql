...
BEGIN
  ...
  IF preparation.template_version='v1'
    AND NOT (preparation.result_snapshot ? 'schemaVersion')
  THEN
    legacy_contact_fingerprint := encode(
      extensions.digest(
        convert_to(
          format(
            '{"channel":%s,"contactId":%s,"value":%s}',
            to_json('EMAIL'::text)::text,
            to_json(preparation.contact_id::text)::text,
            to_json(resolved.contact_value)::text
          ),
          'UTF8'
        ),
        'sha256'
