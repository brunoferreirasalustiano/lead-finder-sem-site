...
BEGIN
  IF p_pilot_run_id IS NULL
    ...
    OR p_template_id IS DISTINCT FROM 'pilot-email-first-contact'
  ...
  IF snapshot_key_count <> 8
    OR p_result_snapshot->'schemaVersion' IS DISTINCT FROM '2'::jsonb
    OR p_result_snapshot->>'channel' IS DISTINCT FROM 'EMAIL'
    OR p_result_snapshot->>'templateId' IS DISTINCT FROM p_template_id
    OR p_result_snapshot->>'templateVersion' IS DISTINCT FROM p_template_version
    OR p_result_snapshot->'variables' IS DISTINCT FROM '{}'::jsonb
  ...
  IF p_result_fingerprint::text IS DISTINCT FROM expected_result_fingerprint THEN
  ...
  IF resolved.contact_fingerprint::text IS DISTINCT FROM p_result_snapshot->>'contactFingerprint' THEN
  ...
  IF FOUND THEN
    IF existing.operator_principal_id IS DISTINCT FROM p_operator_principal_id
      OR existing.lead_id IS DISTINCT FROM p_lead_id
      OR existing.contact_id IS DISTINCT FROM p_contact_id
      OR existing.channel IS DISTINCT FROM 'EMAIL'
      OR existing.template_id IS DISTINCT FROM p_template_id
      OR existing.template_version IS DISTINCT FROM p_template_version
      OR existing.payload_fingerprint::text IS DISTINCT FROM p_payload_fingerprint::text
      OR (
        existing.template_version <> 'v1'
        AND (
          existing.result_fingerprint::text IS DISTINCT FROM p_result_fingerprint::text
          OR existing.result_snapshot IS DISTINCT FROM p_result_snapshot
        )
      )
    THEN
...
BEGIN
  ...
  END IF;
  IF attempt_record.operator_principal_id IS DISTINCT FROM p_operator_principal_id THEN
    RAISE EXCEPTION 'manual email attempt principal mismatch' USING ERRCODE='42501';
  END IF;
