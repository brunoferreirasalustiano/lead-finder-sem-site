...
const renderHome = (csrfToken: string, apiBaseUrl: string, error?: string) => page(
  ...
  <form method="post" action="/prepare" autocomplete="off">
    ...
    <button type="submit">Preparar mensagem</button>
  </form>`,
);
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/') {
        sendHtml(response, 200, renderHome(csrfToken, apiBaseUrl));
        return;
      }
      ...
      sendHtml(response, 422, renderHome(csrfToken, apiBaseUrl, code));
    }
  });
