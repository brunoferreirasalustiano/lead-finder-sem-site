...
export function buildApp(db: Database, options: {
  ...
  const manualMessagingRoute=async(reply:Parameters<typeof pilotRoute>[0],action:()=>Promise<unknown>)=>{try{return await action();}catch(error){if(error instanceof ManualMessagingError){const status=error.code==='NOT_FOUND'?404:error.code==='IDEMPOTENCY_CONFLICT'?409:error.code==='WHATSAPP_CLOUD_FORBIDDEN'?403:error.code==='WHATSAPP_CLOUD_UNAVAILABLE'||error.code==='WHATSAPP_CLOUD_AMBIGUOUS'?503:422;return reply.status(status).send({error:'Manual messaging operation failed',code:error.code});}throw error;}};
