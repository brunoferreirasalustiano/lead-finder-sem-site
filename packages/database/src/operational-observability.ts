...
export async function getOperationalSnapshot(db: Database, now = new Date()): Promise<OperationalSnapshot> {
  const rows = await db.execute<{
    ...
    SELECT
      ...
      count(*) FILTER (WHERE status = 'PENDING' AND available_at <= ${now.toISOString()}::timestamptz) AS due_pending_count,
      count(*) FILTER (WHERE status = 'PENDING' AND available_at > ${now.toISOString()}::timestamptz) AS scheduled_pending_count,
      COALESCE(extract(epoch FROM (${now.toISOString()}::timestamptz - min(available_at) FILTER (WHERE status = 'PENDING' AND available_at <= ${now.toISOString()}::timestamptz)) * 1000), 0) AS oldest_pending_age_ms,
  ...
  return { pendingCount: number(row.pending_count), duePendingCount: number(row.due_pending_count), scheduledPendingCount: number(row.scheduled_pending_count), oldestPendingAgeMs: Math.max(0, Math.round(number(row.oldest_pending_age_ms))),
