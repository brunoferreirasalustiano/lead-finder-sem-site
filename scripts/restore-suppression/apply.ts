...
async function resolveEntries(sql: ReturnType<typeof connect>, manifest: SuppressionManifest): Promise<{ resolved: Resolved[]; unresolved: number }> {
  ...
  for (const entry of manifest.entries) {
    const rows = entry.leadId
      ? await sql<{id:string;is_blocked:boolean;do_not_contact:boolean;crm_stage:string|null;has_opt_out:boolean}[]>`SELECT l.id::text,l.is_blocked,l.do_not_contact,l.crm_stage,EXISTS(SELECT 1 FROM campaign_opt_outs o WHERE o.lead_id=l.id AND o.channel IS NOT DISTINCT FROM ${entry.channel ?? null}) has_opt_out FROM leads l WHERE l.id=${entry.leadId}::uuid OR (l.osm_type=${entry.stableIdentity?.osmType ?? ''} AND l.osm_id=${entry.stableIdentity?.osmId ?? ''})`
  ...
  try {
    ...
    await sql.begin(async (tx) => {
      ...
      for (const item of locked.resolved) {
        ...
        await tx`UPDATE campaign_recipients SET state=CASE WHEN ${entry.suppressionType.startsWith('OPT_OUT')} THEN 'OPT_OUT' ELSE 'BLOQUEADO' END,version=version+1,updated_at=now() WHERE lead_id=${leadId}::uuid AND state IN ('PENDENTE','ELEGIVEL','EM_ANDAMENTO')`;
        await tx`UPDATE campaign_attempts a SET state='BLOQUEADA',version=a.version+1,updated_at=now() FROM campaign_recipients r WHERE a.recipient_id=r.id AND r.lead_id=${leadId}::uuid AND a.state IN ('PENDENTE','APROVADA')`;
        await tx`UPDATE campaign_outbox o SET status='FAILED' WHERE o.status='PENDING' AND EXISTS(SELECT 1 FROM campaign_recipients r WHERE r.id=o.aggregate_id AND r.lead_id=${leadId}::uuid)`;
      }
