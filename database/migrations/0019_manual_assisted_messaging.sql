CREATE TABLE IF NOT EXISTS contact_channel_authorizations (
  ...
  granted_at timestamptz NOT NULL, revoked_at timestamptz, recorded_by text NOT NULL CHECK (char_length(btrim(recorded_by)) BETWEEN 1 AND 100),
  created_at timestamptz NOT NULL DEFAULT now(), CHECK (revoked_at IS NULL OR revoked_at >= granted_at),
...
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM pg_roles WHERE rolname='anon') THEN EXECUTE 'REVOKE ALL ON contact_channel_authorizations,pilot_manual_message_preparations,pilot_manual_message_events FROM anon';EXECUTE 'REVOKE ALL ON FUNCTION reject_manual_messaging_history_mutation() FROM anon';END IF;
 IF EXISTS(SELECT 1 FROM pg_roles WHERE rolname='authenticated') THEN EXECUTE 'REVOKE ALL ON contact_channel_authorizations,pilot_manual_message_preparations,pilot_manual_message_events FROM authenticated';EXECUTE 'REVOKE ALL ON FUNCTION reject_manual_messaging_history_mutation() FROM authenticated';END IF;
 IF EXISTS(SELECT 1 FROM pg_roles WHERE rolname='service_role') THEN EXECUTE 'GRANT SELECT,INSERT ON contact_channel_authorizations,pilot_manual_message_preparations,pilot_manual_message_events TO service_role';END IF;
END $$;
