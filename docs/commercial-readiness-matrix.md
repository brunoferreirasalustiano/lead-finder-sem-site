...
Antes de qualquer exposição pública, `/health`, `/health/live`, `/health/ready`, `/internal/operational-snapshot` e os demais endpoints internos exigem autenticação/autorização de perímetro. Esta PR sanitiza respostas, mas não implementa autenticação pública. Restore sem reconciliação de exclusões/opt-outs posteriores ao backup e qualquer `SHADOW_MODE_ENABLED=false` são `NO-GO`.
