...
const postLegacyLowerBound = '0042_';
const migration0049Name = '0049_precontact_email_existing_duplicate_hardening.sql';
const admin = postgres(databaseUrl, { max: 1 });

const listMigrations = async () =>
  (await readdir(migrations)).filter((name) => /^\d{4}_.+\.sql$/u.test(name)).sort();

const applyLegacySnapshot = async (sql: ReturnType<typeof postgres>) => {
  for (const file of (await listMigrations()).filter((name) => name < postLegacyLowerBound)) {
    await sql.unsafe(await readFile(new URL(file, migrations), 'utf8'));
  }
};

const applyPostLegacyMigrations = async (sql: ReturnType<typeof postgres>) => {
  for (const file of (await listMigrations()).filter(
    (name) => name >= postLegacyLowerBound && name <= migration0049Name,
  )) {
    await sql.unsafe(await readFile(new URL(file, migrations), 'utf8'));
  }
};

const ensureMigrationRuntimeRole = async () => {
  const row = (await admin<{ exists: boolean }[]>`
    SELECT EXISTS(SELECT 1 FROM pg_roles WHERE rolname = 'lead_finder_api_role') AS exists
  `)[0]!;
  if (row.exists) return false;

  await admin.unsafe('CREATE ROLE lead_finder_api_role NOLOGIN');
  return true;
};
...
const createDatabase = async (name: string) => {
  ...
  return { url, sql: postgres(url, { max: 1 }) };
};
...
const identity = (key: Buffer, email: string) =>
  createHmac('sha256', key).update(email.trim().toLowerCase(), 'utf8').digest('hex');
...
const buildManifest = (
  ...
  const content = manifestContentSchema.parse({
    schemaVersion: '1.0',
    runId: randomUUID(),
    logicalOrigin: 'DATABASE_PRE_RESTORE',
    cutoffAt: '2026-08-10T12:00:00.000Z',
    entries: [],
    counts: {
      total: 0,
      byType: { IS_BLOCKED: 0, DO_NOT_CONTACT: 0, CRM_NAO_CONTATAR: 0, OPT_OUT_GLOBAL: 0, OPT_OUT_CHANNEL: 0 },
    },
    precontactPermanent: {
...
const seedLegacyBounce = async (sql: ReturnType<typeof postgres>, suffix: string) => {
  ...
  const bouncedEmail = `legacy-${suffix}@example.test`;
  const replacementEmail = `replacement-${suffix}@example.test`;
  const eventFingerprint = digest(`pre0048-legacy-${suffix}`);
  const occurredAt = '2026-08-08T15:00:00.000Z';

  await sql`
    INSERT INTO leads(id,osm_type,osm_id,name,category,score,status,is_closed)
    VALUES(${leadId}::uuid,'node',${`legacy-${suffix}`},'Legacy restore fixture','integration',1,'SEM_SITE_CADASTRADO',false)
  `;
  await sql`
    ...
    VALUES(
      ${contactId}::uuid,${leadId}::uuid,'EMAIL',${bouncedEmail},${bouncedEmail},'INTEGRATION',1,now(),true,false
    )
  `;
  const binding = (await sql<{ fingerprint: string }[]>`
    SELECT contact_resolution_fingerprint fingerprint FROM lead_contacts WHERE id=${contactId}::uuid
  `)[0]!.fingerprint;
  await sql`
...
try {
  ...
  try {
    await applyLegacySnapshot(unresolved.sql);
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        error instanceof Error && error.message.includes('PRE0048_LEGACY_SUPPRESSION_UNRESOLVED'),
    );

    const proof = await unresolved.sql<{ legacy: number; global_relation: string | null; identity_column: boolean }[]>`
      SELECT
        (SELECT count(*)::int FROM contact_delivery_suppressions
          WHERE event_fingerprint=${fixture.eventFingerprint}::char(64)) legacy,
    ...
    await unresolved.sql.end();
    await dropDatabase(unresolvedName);
  }

  const resolvedName = `leadfinder_restore_pre0048_resolved_${process.pid}`;
  const resolved = await createDatabase(resolvedName);
  try {
    await applyLegacySnapshot(resolved.sql);
    const fixture = await seedLegacyBounce(resolved.sql, 'resolved');
    await resolved.sql`
      UPDATE lead_contacts
      SET original_value=${fixture.replacementEmail},normalized_value=${fixture.replacementEmail},is_valid=true
    ...
    const manifest = buildManifest(key, {
      identityFingerprint: bouncedIdentity,
      reasonCode: 'HARD_BOUNCE',
      operationalSource: 'RESTORE_LEGACY_TEST',
    ...
    `;
    assert.deepEqual(bridge[0], {
      historical_identity: bouncedIdentity,
      global_identity: bouncedIdentity,
      key_digest: manifest.precontactPermanent.keyDigest,
    });

    createdMigrationRuntimeRole = await ensureMigrationRuntimeRole();
    await applyPostLegacyMigrations(resolved.sql);
    ...
    const final = await resolved.sql<{
      ...
      SELECT contact.is_valid,
        ...
        identity.suppressed,
        (SELECT count(*)::int FROM email_precontact_delivery_suppressions
          WHERE event_fingerprint=${fixture.eventFingerprint}::char(64)) events
      FROM lead_contacts contact
      JOIN contact_delivery_suppressions suppression
        ON suppression.contact_id=contact.id AND suppression.lead_id=contact.lead_id
      JOIN lead_finder_private.email_contact_identities identity
        ON identity.identity_fingerprint=suppression.email_precontact_identity_fingerprint
      WHERE contact.id=${fixture.contactId}::uuid
        AND suppression.event_fingerprint=${fixture.eventFingerprint}::char(64)
    `;
    assert.deepEqual(final[0], {
      is_valid: true,
      current_identity: replacementIdentity,
      historical_identity: bouncedIdentity,
      suppressed: true,
      events: 1,
    });
    ...
    await dropDatabase(resolvedName);
  }

  process.stdout.write(
    JSON.stringify({
      gate: 'RESTORE_PRE0048_LEGACY',
      result: 'PASS',
      exactManifestBridge: true,
      replacementAddressNotSuppressed: true,
      unresolvedFailsClosed: true,
      migrationBoundary0041: true,
    }) + '\n',
  );
} finally {
  if (createdMigrationRuntimeRole) {
    await admin.unsafe('DROP ROLE IF EXISTS lead_finder_api_role');
  }
  await admin.end();
}
