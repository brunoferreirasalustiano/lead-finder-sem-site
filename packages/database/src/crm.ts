...
async function replay(tx: Tx, scope: string, key: string, payload: unknown) {
...
export async function createOpportunity(db: Database, leadId: string, input: OpportunityCreateInput) {
  return db.transaction(async (tx) => {
    ...
    const old = await replay(tx, scope, input.idempotencyKey, input); if (old) return old;
...
export const listOpportunities = (db: Database, leadId: string) => db.select().from(crmOpportunities).where(eq(crmOpportunities.leadId, leadId)).orderBy(desc(crmOpportunities.updatedAt), desc(crmOpportunities.id));
...
export async function updateOpportunity(db: Database, opportunityId: string, input: OpportunityUpdateInput) {
  return db.transaction(async (tx) => {
    ...
    await remember(tx, scope, input.idempotencyKey, input, 'opportunity', row.id, row); return row;
  });
}
...
export async function changeCrmStage(db: Database, leadId: string, input: CrmStageChangeInput) {
  return db.transaction(async (tx) => {
    ...
    await remember(tx, scope, input.idempotencyKey, input, 'lead', leadId, row); return row;
  });
}
...
export async function updateCrmAssignment(db: Database, leadId: string, input: { owner?: string | null; priority?: CrmPriority; nextActionAt?: Date | null; actor: string; expectedVersion: number; idempotencyKey: string }) {
  return db.transaction(async (tx) => {
    const scope = `lead:${leadId}:assignment`; const old = await replay(tx, scope, input.idempotencyKey, input); if (old) return old;
...
export async function addNote(db: Database, leadId: string, input: NoteCreateInput & { opportunityId?: string }) {
  return db.transaction(async (tx) => { const scope = `lead:${leadId}:note`; const old = await replay(tx, scope, input.idempotencyKey, input); if (old) return old; await requireCommercialLead(tx, leadId);
    ...
    await event(tx, { leadId, opportunityId: input.opportunityId, eventType: 'NOTE_ADDED', actor: input.actor, newValue: row }); await remember(tx, scope, input.idempotencyKey, input, 'note', row.id, row); return row; });
}
export const listNotes = (db: Database, leadId: string) => db.select().from(crmNotes).where(eq(crmNotes.leadId, leadId)).orderBy(desc(crmNotes.createdAt), desc(crmNotes.id));

export async function addTag(db: Database, leadId: string, name: string, input: { actor: string; idempotencyKey: string }) {
  return db.transaction(async (tx) => { const payload = { name, ...input }; const scope = `lead:${leadId}:tag:add`; const old = await replay(tx, scope, input.idempotencyKey, payload); if (old) return old; await requireCommercialLead(tx, leadId);
    ...
    const result = { ...tag, leadId }; await event(tx, { leadId, eventType: 'TAG_ADDED', actor: input.actor, newValue: result }); await remember(tx, scope, input.idempotencyKey, payload, 'tag', tag.id, result); return result; });
}
export async function removeTag(db: Database, leadId: string, name: string, input: { actor: string; idempotencyKey: string }) {
  return db.transaction(async (tx) => { const payload = { name, ...input }; const scope = `lead:${leadId}:tag:remove`; const old = await replay(tx, scope, input.idempotencyKey, payload); if (old) return old; await requireCommercialLead(tx, leadId);
    ...
    await event(tx, { leadId, eventType: 'TAG_REMOVED', actor: input.actor, newValue: result }); await remember(tx, scope, input.idempotencyKey, payload, 'tag', tag.id, result); return result; });
}
export const listTags = (db: Database, leadId: string) => db.select({ id: crmTags.id, name: crmTags.name, createdAt: crmLeadTags.createdAt }).from(crmLeadTags).innerJoin(crmTags, eq(crmTags.id, crmLeadTags.tagId)).where(eq(crmLeadTags.leadId, leadId)).orderBy(asc(crmTags.name));

export async function createTask(db: Database, leadId: string, input: TaskCreateInput & { opportunityId?: string }) {
  return db.transaction(async (tx) => { const scope = `lead:${leadId}:task:create`; const old = await replay(tx, scope, input.idempotencyKey, input); if (old) return old; await requireCommercialLead(tx, leadId);
    ...
    await event(tx, { leadId, taskId: row.id, opportunityId: input.opportunityId, eventType: 'TASK_CREATED', actor: input.actor, newValue: row }); await remember(tx, scope, input.idempotencyKey, input, 'task', row.id, row); return row; });
}
export const listTasks = (db: Database, leadId: string) => db.select().from(crmTasks).where(eq(crmTasks.leadId, leadId)).orderBy(asc(crmTasks.dueAt), asc(crmTasks.id));
async function mutateTask(db: Database, taskId: string, input: TaskCompleteInput | TaskRescheduleInput, kind: 'complete' | 'reschedule') {
  return db.transaction(async (tx) => { const scope = `task:${taskId}:${kind}`; const old = await replay(tx, scope, input.idempotencyKey, input); if (old) return old;
...
export const listTimeline = (db: Database, leadId: string) => db.select().from(crmTimelineEvents).where(eq(crmTimelineEvents.leadId, leadId)).orderBy(desc(crmTimelineEvents.createdAt), desc(crmTimelineEvents.id));
