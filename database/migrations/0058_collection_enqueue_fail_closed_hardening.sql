-- Harden the 0057 enqueue boundary without rewriting the already-applied
-- migration. Every authorization field is validated null-safely before any
-- batch or collection-job write, and the city identity matches the shared
-- collectionCityId contract.
...
BEGIN
  ...
  -- Every authorization field is explicit and null-safe. Missing JSON keys,
  -- JSON nulls, wrong types, and wrong values all reject before INSERT.
  ...
  -- This is the SQL equivalent of collectionCityId(city, state): city is
  -- lowercased, Portuguese combining accents are removed, separators become
  -- hyphens, while state is lowercased and non-alphanumerics are removed.
  payload_city_normalized := regexp_replace(
    regexp_replace(
      lower(translate(payload_city,
