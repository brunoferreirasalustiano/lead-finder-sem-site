...
export const safeManualEmailDeliveryDto = (
  result: Awaited<ReturnType<typeof sendPreparedManualEmail>>,
) => ({
  state: result.state,
  provider: result.provider,
  replayed: result.replayed,
  attemptId: result.attemptId,
  ...(result.messageIdFingerprint
    ? { messageIdFingerprint: result.messageIdFingerprint }
    : {}),
...
const safeDailyWhatsappRecentCnpjOpportunityDto = (
  candidate: Awaited<ReturnType<typeof listDailyWhatsappRecentCnpjOpportunities>>[number],
) => ({
  leadId: candidate.lead_id,
  businessName: (candidate.lead_name ?? 'empresa').slice(0, 200),
  city: candidate.city.slice(0, 100),
  category: candidate.category.slice(0, 100),
  whatsapp: candidate.whatsapp_value,
  whatsappSource: candidate.whatsapp_source,
  whatsappEvidence: candidate.whatsapp_evidence,
  websiteStatus: candidate.website_status.slice(0, 64),
  qualificationStatus: candidate.qualification_status.slice(0, 64),
  businessIdentityConfirmed: candidate.business_identity_confirmed,
  businessActivePass: candidate.business_active_pass,
  cnpj: candidate.cnpj,
  cnpjOpeningDate: candidate.cnpj_opening_date,
  cnpjRegistrationStatus: candidate.cnpj_registration_status,
  cnpjSource: candidate.cnpj_source,
  opportunityState: candidate.business_identity_confirmed && candidate.business_active_pass
    ? 'WHATSAPP_CNPJ_OPPORTUNITY_READY'
...
export function buildApp(db: Database, options: {
  ...
  app.get('/internal/dailywhatsapp/cnpj-opportunities', async (request, reply) => {
    reply.header('cache-control', 'no-store');
    if (options.whatsappOpportunityReviewEnabled !== true) {
      return reply.status(404).send({ error: 'Not found', code: 'NOT_FOUND' });
    }
    ...
    const query = dailyWhatsappRecentCnpjQuerySchema.safeParse(request.query);
    if (!query.success) return reply.status(400).send({ error: 'Invalid CNPJ opportunity query', code: 'INVALID_REQUEST' });
    try {
      const candidates = await contractQueries.listDailyWhatsappRecentCnpjOpportunities(db, {
        city: query.data.city,
        ...(query.data.category ? { category: query.data.category } : {}),
        openedSince: query.data.openedSince,
        limit: query.data.limit,
      });
      if (candidates.length > query.data.limit) {
        return reply.status(503).send({ error: 'Service unavailable', code: 'OPPORTUNITY_RESULT_OVER_LIMIT' });
  ...
  const cancelManualMessageSchema=z.object({observation:z.string().max(500).optional()}).strict();
  ...
  const manualMessagingRoute=async(reply:Parameters<typeof pilotRoute>[0],action:()=>Promise<unknown>)=>{try{return await action();}catch(error){if(error instanceof ManualMessagingError){const consumedScope=error.code==='WHATSAPP_TEST_SCOPE_CONSUMED';const status=consumedScope?409:error.code==='NOT_FOUND'?404:error.code==='IDEMPOTENCY_CONFLICT'?409:error.code==='WHATSAPP_CLOUD_FORBIDDEN'?403:error.code==='EMAIL_CONSUMER_UNAVAILABLE'||error.code==='WHATSAPP_CLOUD_UNAVAILABLE'||error.code==='WHATSAPP_CLOUD_AMBIGUOUS'?503:422;return reply.status(status).send(consumedScope?{error:'WhatsApp test scope already consumed',code:error.code,retryAllowed:false}:{error:'Manual messaging operation failed',code:error.code});}throw error;}};
  ...
  app.get('/manual-message-preparations/:id/whatsapp-link',async(request,reply)=>{const id=parseId(request.params);if(!id.success)return reply.status(400).send({error:'Invalid WhatsApp link request',code:'INVALID_REQUEST'});return manualMessagingRoute(reply,async()=>{const result=await getPreparedWhatsAppLink(db,id.data,authorizationContextFor(request));request.log.info({event:'manual_whatsapp_redirected',preparationId:id.data,principalId:request.principal!.id},'manual_whatsapp_redirected');return reply.header('cache-control','no-store').header('pragma','no-cache').header('referrer-policy','no-referrer').redirect(result.link,302);});});
  app.post('/manual-message-preparations/:id/cancel',async(request,reply)=>{const id=parseId(request.params);const key=idempotencyKey(request.headers);const body=cancelManualMessageSchema.safeParse(request.body);if(!id.success||!key.success||!body.success)return reply.status(400).send({error:'Invalid cancellation request',code:'INVALID_REQUEST'});return manualMessagingRoute(reply,async()=>{const result=await cancelManualPreparation(db,id.data,{...body.data,idempotencyKey:key.data},authorizationContextFor(request));request.log.info({event:'manual_message_cancelled',preparationId:id.data,state:result.state,principalId:request.principal!.id,replayed:result.replayed},'manual_message_cancelled');return reply.status(creationStatus(result.replayed)).send(result);});});
