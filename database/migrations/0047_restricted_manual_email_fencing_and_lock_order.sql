...
-- A retry/observer must never manufacture a terminal outcome merely because a
-- fixed lease elapsed. The original sender remains authoritative until it
-- persists DELIVERED/FAILED/AMBIGUOUS. This is deliberately fail-closed: a
-- crashed sender leaves the attempt IN_PROGRESS for explicit operational
-- reconciliation rather than allowing a duplicate delivery.
...
-- Legacy manual-message transitions acquire manual-message-preparation:<id>
-- before locking the preparation row. Acquire that same lifecycle lock first
-- here too; otherwise OPENED can invert the order against CANCELLED or
-- CONTACT_CONFIRMED and deadlock in PostgreSQL.
...
BEGIN
  ...
  -- Revalidation remains inside the same statement. Any rejection rolls the
  -- provisional OPENED insert back atomically.
