...
const aggregate = { pending_count: 4, oldest_pending_age_ms: 301000, claimed_count: 1, published_count: 8, exhausted_count: 2, retry_count: 3, expired_lease_count: 1, recovered_lease_count: 2, dead_letter_count: 2, throughput_recent: 5 };
describe('operational snapshot', () => {
  it('returns stable, low-cardinality database state and restart-safe fields', async () => {
    ...
    await expect(getOperationalSnapshot(db, new Date('2030-01-01T00:00:00.000Z'))).resolves.toEqual({ pendingCount: 4, oldestPendingAgeMs: 301000, claimedCount: 1, publishedCount: 8, exhaustedCount: 2, retryCount: 3, staleAckCount: 0, expiredLeaseCount: 1, recoveredLeaseCount: 2, deadLetterCount: 2, throughputRecent: 5, averageDurationMs: 0, errorsByReason: {} });
  });
